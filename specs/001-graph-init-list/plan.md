...
| IV | Test-First Development | PASS | Integration tests required; live Git repos in `testdata/`; unique names for parallel runs; `t.Cleanup` for teardown |
...
| IV | Test-First Development | PASS | Live repos in `testdata/` (git-ignored); unique subdir per test; `t.Cleanup` removes on success; path logged on failure for inspection |
