...
- [ ] T001 [P] Add `TreeNode`, `TreeOptions`, `TreeResult` (type alias for `TreeNode`), and `TreeAllResult` types to src/graph/node.go per contracts/go-api.md — `TreeNode` has `Name`, `Type`, `ID`, `Children` fields; `TreeOptions` has `Depth` and `HasDepth` fields; `TreeAllResult` has `Graphs` and `Warnings` fields
- [ ] T002 [P] Add `tree` command case to the CLI switch in src/cmd/grif/main.go (call placeholder `runTree` that prints "not implemented") and update `printUsage` to include the `tree` command in the commands list
...
- [ ] T005 Implement `Tree(repoPath, path, opts)` function in src/graph/graph.go — validate depth (negative → error), parse path (graph name + optional segments), validate graph name via `ValidateGraphName`, resolve root tree via `gitops.ResolveRootTree`, walk to target subtree/blob if path has segments, build recursive `TreeNode` children respecting depth limit, sort children alphabetically at each level, return `*TreeResult`
- [ ] T006 Implement `TreeAll(repoPath, opts)` function in src/graph/graph.go — validate depth (negative → error), list all graphs via `gitops.ListRefsByPrefix`, error if no graphs found, sort alphabetically, call `Tree` for each graph, collect warnings for failed graphs (partial success per FR-019), return `*TreeAllResult`
...
- [ ] T007 [US1] Write tests verifying box-drawing output format in src/graph/graph_test.go — verify `├──` for non-last siblings, `└──` for last siblings, `│` for continuation lines, blob annotation format `(blob, <8-char-hash>)` with two-space separator, tree nodes show name only, entries sorted alphabetically at each level, empty graph shows only graph name, graph not found produces error

### Implementation for User Story 1

- [ ] T008 [US1] Implement `runTree` function in src/cmd/grif/main.go — parse positional argument, call `graph.Tree`, implement box-drawing renderer that recursively formats `TreeNode` hierarchy with correct prefix tracking (`├── `/`└── ` connectors, `│   `/`    ` continuations), write output to stdout, handle errors with `printError` and exit code 1
- [ ] T009 [US1] Add `printTreeUsage` help text function in src/cmd/grif/main.go
...
- [ ] T010 [US2] Write tests for no-argument mode in src/graph/graph_test.go — verify multiple graphs displayed in alphabetical order separated by blank line, single graph displays without trailing blank line, no graphs produces error, partial success emits warnings to stderr and exits 0

### Implementation for User Story 2

- [ ] T011 [US2] Add no-argument mode to `runTree` in src/cmd/grif/main.go — when no positional argument provided, call `graph.TreeAll`, render each graph's tree separated by blank lines, emit warnings to stderr for failed graphs per FR-019, exit 0 on partial success
...
- [ ] T012 [US3] Write tests for subtree mode in src/graph/graph_test.go — verify subtree rooted at intermediate tree node, subtree resolving to blob displays single line with `(blob, hash)` format, path not found produces error, deeply nested subtree renders correctly

### Implementation for User Story 3

- [ ] T013 [US3] Verify subtree handling in `runTree` in src/cmd/grif/main.go — `graph.Tree` already handles subtree paths via path walking; verify the CLI renderer correctly uses the last path segment as root label (not the full path); add test coverage for blob-at-path single-line display
...
- [ ] T014 [US4] Write tests for `--depth` flag in src/graph/graph_test.go — verify depth 0 shows root only, depth 1 shows root + immediate children (trees without their children), depth exceeds actual depth shows full tree, negative depth produces error, depth works with no-argument (all-graphs) mode applying uniformly

### Implementation for User Story 4

- [ ] T015 [US4] Add `--depth` flag parsing to `runTree` in src/cmd/grif/main.go — parse integer value from `flagValue("--depth")`, set `TreeOptions.Depth` and `TreeOptions.HasDepth`, pass to `graph.Tree` or `graph.TreeAll`; validate parse errors in CLI before calling module
- [ ] T016 [US4] Update `positionalArgs` in src/cmd/grif/main.go to skip `--depth` and its value argument so they are not treated as positional arguments
...
- [ ] T017 [US5] Write tests for `--json` output mode in src/graph/graph_test.go — verify single graph produces JSON object with `name`, `type`, `id`, `children` fields, all-graphs mode produces JSON array, blob node omits `children` field, empty tree has empty `children` array, `--json --depth N` limits JSON depth, all-graphs with warnings produces object with `graphs` and `warnings` fields, output is valid parseable JSON

### Implementation for User Story 5

- [ ] T018 [US5] Add `--json` output mode to `runTree` in src/cmd/grif/main.go — when `--json` is set: single graph/subtree marshals `TreeResult` directly; all-graphs without warnings marshals `Graphs` slice as plain array; all-graphs with warnings marshals full `TreeAllResult` object; JSON errors use `printError` format on stderr
...
- [ ] T019 [P] Update README.md to document the `tree` command, its flags (`--depth`, `--json`), and usage examples per Constitution README.md requirements
- [ ] T020 Run quickstart.md scenarios manually to validate end-to-end behavior
- [ ] T021 Run `make lint` and `make test` to verify all checks pass
...
- The box-drawing renderer lives in the CLI (`main.go`), not the module — it is output formatting only
- `TreeResult = TreeNode` is a type alias, so JSON output naturally produces the correct nested structure
- All-graphs JSON: plain array `[...]` when no warnings; object `{"graphs":[...],"warnings":[...]}` when warnings present
