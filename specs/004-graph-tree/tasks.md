...
- [ ] T001 [P] Add `TreeItem`, `TreeOptions`, `TreeResult` (type alias for `TreeItem`), and `TreeAllResult` types to src/graph/node.go per contracts/go-api.md — `TreeItem` has `Name`, `Type`, `ID`, `Children` fields; `TreeOptions` has `Depth` and `HasDepth` fields; `TreeAllResult` has `Graphs` and `Warnings` fields
...
- [ ] T011 [US2] Add no-argument mode to `runTree` in src/cmd/grif/main.go — when no positional argument provided, call `graph.TreeAll`, call `graph.FormatTreeAll` to render each graph's tree separated by blank lines, emit warnings to stderr for failed graphs per FR-019, exit 0 on partial success
...
- [ ] T018 [US5] Add `--json` output mode to `runTree` in src/cmd/grif/main.go — when `--json` is set: single graph/subtree marshals `TreeResult` directly; all-graphs marshals full `TreeAllResult` object (always wrapper with `graphs` array); JSON errors use `printError` format on stderr
...
- [ ] T020 Write a benchmark test `BenchmarkTree1000Nodes` in src/graph/graph_test.go — creates a graph with 1,000 blob nodes distributed across multiple tree levels and benchmarks `graph.Tree`; validates SC-001 performance goal (< 1 second on local repository)
- [ ] T021 Run quickstart.md scenarios manually to validate end-to-end behavior
- [ ] T022 Run `make lint` and `make test` to verify all checks pass
